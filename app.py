...
def proteger_rutas():
    ...
    public_path_prefixes = (
        ...
        '/static/uploads/productos/',  # <--- ESTA ES LA CLAVE
        '/static/audio/',
...
def obtener_productos_por_palabra_clave(palabra_clave, config=None, limite=150, contexto_ia=None):
    ...
    try:
        ...
        if contexto_ia and contexto_ia.startswith('CATEGORIA_'):
            ...
            termino_cat = f"%{categoria}%"
            cursor.execute(query_categoria, (termino_cat, termino_cat, limite))
            resultados = cursor.fetchall()
            
            if resultados:
                app.logger.info(f"✅ Encontrados {len(resultados)} productos por categoría IA")
                cursor.close()
                conn.close()
                return resultados
...
def webhook():
    try:
        ...
        # 2. Ignorar actualizaciones de estado
        ...
        # Limpieza de caché de duplicados
        now_ts = time.time()
        for h, ts in list(processed_messages.items()):
            if now_ts - ts > 3600:
                del processed_messages[h]

        # 8. Procesamiento de contenido
        texto = ''
        es_imagen = es_audio = es_archivo = False
        imagen_base64 = public_url = transcripcion = audio_url = None
        ...
        # 10. EXTRAER HISTORIAL FINAL (Única vez)
        # Lo extraemos después de registrar la nueva para que incluya el mensaje actual si es necesario
        historial_final = []
        try:
            from services import obtener_historial
            historial_final = obtener_historial(numero, limite=10, config=config)
        except Exception as e:
            app.logger.error(f"❌ Error crítico obteniendo historial_final: {e}")
        # --------------------------------

        # 11. Guardar mensaje inmediatamente
        try:
            if es_audio:
                guardar_mensaje_inmediato(
                    numero, texto, config, 
        ...
        # 12. Delegar con el historial ya cargado
        # Añadimos historial_final a los argumentos
        processed_ok = procesar_mensaje_unificado(
            ...
            incoming_saved=True,
            historial_final=historial_final  # <--- Inyectamos la variable aquí
        )
...
def generar_respuesta_deepseek(numero, texto, precios, historial_final, config, incoming_saved=False, es_audio=False, es_imagen=False, imagen_base64=None, transcripcion=None, catalog_list=None, texto_catalogo=None, producto_aplica="NO_APLICA"):
    try:
        ...
        contexto_productos = ""
        if hay_productos:
            contexto_productos = "\nLISTA DE PRECIOS/PRODUCTOS (Usa este formato: *Nombre* - $Precio):\n"
            for p in items_catalogo[:15]:
                nombre = p.get('servicio') or p.get('modelo') or p.get('nombre')
                precio = p.get('precio_menudeo') or p.get('precio') or "Consultar"
                contexto_productos += f"• *{nombre}*: ${precio}\n"

        # Análisis de repetición: Si ya hay mensajes de la IA en el historial, le prohibimos presentarse
        ya_saludo = any(h.get('enviado_por') == 'ia' for h in historial_final)
        
        if not ya_saludo:
            identidad_instruccion = f"Identifícate como {ia_nombre} de {negocio_nombre}."
        else:
            identidad_instruccion = "IMPORTANTE: Ya saludaste anteriormente. NO repitas tu nombre ni el del negocio. Ve directo al grano."
        ...
        REGLAS DE FORMATO:
        1. Usa *negritas* para resaltar productos, precios y puntos clave.
        2. Usa viñetas (•) para listas.
        3. Separa párrafos con saltos de línea.
        4. Responde de forma natural y breve (máximo 3-4 líneas).
        ...
        if historial_final:
            for h in historial_final[-8:]:
                ...
                if contenido: # Solo añadir si hay texto
        ...
        if es_audio and mensaje_para_cliente:
            try:
                ...
                tono_voz = config.get('tono_voz', 'nova')
                filename = f"resp_{numero}_{int(time.time())}"
                audio_url_publica = texto_a_voz(mensaje_para_cliente, filename, config, voz=tono_voz)
        ...
        if intent == "PASAR_ASESOR" or notify_asesor is True or usuario_quiere_humano:
            saludos_simples = ["hola", "buen dia", "buenas tardes", "buenas noches", "hey", "que tal"]
            mensaje_limpio = texto_actual.lower().strip()
            es_solo_saludo = mensaje_limpio in saludos_simples
            
            # Solo evitamos transferir si es un saludo simple SIN queja
            if es_solo_saludo and not usuario_quiere_humano:
                app.logger.info(f"🤖 LOVA ignora petición de transferencia: Es solo un saludo.")
            else:
                ...
                # CORRECCIÓN: Usamos mensaje_para_cliente (que viene de la IA) o texto por defecto
                msg_final = mensaje_para_cliente or "Un momento, te comunico con un asesor."
                ...
                if numero.startswith('tg_'):
                    ...
                    if telegram_token:
                        ...
                        send_telegram_message(chat_id, msg_final, telegram_token) 
                else:
                    enviar_mensaje(numero, msg_final, config) 
                
                from whatsapp import registrar_respuesta_bot
                registrar_respuesta_bot(numero, texto_actual, msg_final, config, incoming_saved=incoming_saved)
                return True
...
3) **CLASIFICACIÓN INTELIGENTE DE PRODUCTOS:**
   - SIEMPRE lee TODOS los campos: categoria, subcategoria, linea, servicio, modelo, descripcion
...
5) Si el usuario dice solo "Hola", responde: "Hola, ¿en qué puedo ayudarte?" (breve).
...
- sku: código único
- categoria: tipo principal (primera fuente de verdad para clasificar)
- subcategoria: código interno (NO uses esto solo)
- linea: subtipo del producto
...
MÉTODO DE CLASIFICACIÓN (IMPORTANTE):
1. Usuario menciona palabra clave (ej: "sillas", "cursos", "herramientas")
2. Para cada producto, busca esa palabra en: categoria, servicio, linea, descripcion
3. Compara SIN importar mayúsculas (mentalmente convierte todo a minúsculas)
...
with app.app_context():
    ...
    # NOTA: Aquí NO activamos el scheduler para evitar que se ejecute doble en modo debug
    for nombre, config in NUMEROS_CONFIG.items():
            _ensure_performance_indexes(config)
...
if __name__ == "__main__":
    ...
    # 2. Ejecutamos la app. 
    # Añadimos use_reloader=False para que no arranque el scheduler dos veces por accidente
    app.run(
        host="0.0.0.0",
        port=args.port,
        debug=True,
