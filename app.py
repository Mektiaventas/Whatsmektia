...
def dashboard_conversaciones_data():
    ...
    - labels/values: por día, usando la lógica de 24 horas en la tabla 'conversaciones'.
    """
    try:
        config = obtener_configuracion_por_host()
        period = request.args.get('period', 'week')
        ...
        if period == '3months':
            ...
            # 2. Consulta para obtener el conteo de CONVERSACIONES INICIADAS por día
            # Se usa la tabla 'conversaciones' con la lógica de 24h, ya que 'contactos' no tiene historial diario.
            sql = """
        ...
        # --- Comportamiento por defecto (semana/mes) ---
            ...
            # Consulta para obtener el conteo de CONVERSACIONES INICIADAS por día en el periodo
            # Usando la tabla 'conversaciones' con la lógica de 24h
            cursor.execute("""
                SELECT DATE(c1.timestamp) as dia, COUNT(DISTINCT c1.numero) as cnt
                FROM conversaciones c1
                WHERE c1.timestamp >= %s
        ...
        # 3. Chats activos: distinct numero con mensaje en las últimas 24h
        ...
        # 4. Plan info (ya actualizado para usar contactos.conversaciones)
        plan_info = None
        try:
            au = session.get('auth_user')
            if au and au.get('user'):
